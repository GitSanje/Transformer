...
#https://chatgpt.com/c/db19c03c-6976-4c29-ae8b-f0ed36e0f515
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        #Apply this sin to even postions
...
class FeedForwardBlock(nn.Module):
    ...
    def forward(self, x):
        # (batch, seq_len, d_model) -->l1 --> ()
        return self.linear_2(self.dropout(torch.relu(self.linear_1(x))))
    
    
class MultiheadAttentionBlock(nn.Module):
    def __init__(self, d_model: int, h:int, dropout: float ) -> None: # h -> heads
        super().__init__()
        self.d_model = d_model
        self.h = h
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        attentions_scores.softmax(dim=-1)  # (Batch, h , seq_len, seq_len)
    ...
    def forward(self, q, k ,v, mask):
        ...
        query = query.view(query.shape[0],query.shape[1], self.h, self.d_k).transpose(1,2) #(batch, n_h, seq_len, d_k) head_dim->d_k
        key = key.view(key.shape[0],key.shape[1],self.h, self.d_k).transpose(1,2)
        value = value.view(value.shape[0],value.shape[1],self.h, self.d_k).transpose(1,2)
        
        x, self.attention_scores = MultiheadAttentionBlock.attention(query,key, value, mask, self.dropout)
        
        #(batch, n_h, seq_len , d_k) --> (batch, seq_len,n_h  , d_k)  --> (batch, seq_len, d_model) 
...
# N => no of decoder block
def build_transformer(src_vocab_size: int, tgt_vocab_size:int, 
                      src_seq_len: int, tgt_seq_len: int, d_model: int =512, N: int=6 , h=8,
